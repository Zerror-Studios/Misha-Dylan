  ...
  return (
    ...
    >
      <div className="w-[80%] max-sm:w-full px-6 max-sm:px-[20px] flex flex-col gap-20">
        <h2 className="text-center uppercase Font_Q flex flex-col justify-center items-center max-sm:justify-start max-sm:mt-10 COLOR_TEXT_RED">
          Travel Tips
        ...
        <div className="space-y-6 w-full">
          {faqs.map((faq) => (
            <div
              key={faq.id}
              className="border-b border-[#01634221] pb-6 group relative"
            >
              {/* BUTTON */}
              <button
              ...
              >
                <div className="mt-4 Font_YV text-[18px] text-[#016342]">
                  {faq.answer}
              ...
              {/* HOVER LINE */}
              <div className="w-0 h-px bg-[#016342] group-hover:w-full absolute bottom-0 left-0 transition-all duration-200"></div>
            </div>
          ))}
        </div>
      </div>
